...
Our goal is to generate samples whose distribution is sufficiently \textit{close} to $\pi$ that estimating expectations like $\E[\phi(X)]$ where $\phi$ is a bounded function. 
...
Hyvareinen \cite{hyvarinen_estimation_2005} noticed that if $p_\pi$ is decays sufficiently fast at infinity, such that for any $\theta \in \Theta$,
\(
...
\[
\min_{\theta \in \Theta} \E_{X \sim \pi, Z \sim \mathcal{N}(0, \sigma^2I)} \left[ \|s_\theta(X + Z) -  \frac{\|Z\|_2^2}{2\sigma^2}\|_2^2\right].
\]
...
Gaussian smoothing addresses both 

Convolution with a gaussian adds regularity so the minimal requirement for LMC are met. Moreover, if the data is perturbed with a gaussian with high enough variance, then a few samples will land in them, thus improving the score estimation in those regions.
...
Annealed Langevin dynamics is a method inspired by simulated annealing in the sense that a decreasing sequence of 
