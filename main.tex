...
\begin{equation}
    \frac{d}{dt}\KL(p_t || \pi) = \int_{\mathbb{R}^d} {\color{BrickRed} \frac{\partial p_t}{\partial t}}(x)\ln\frac{p_t(x)}{\pi(x)}dx.
    \label{eq:kl-deriv}
\end{equation}

From \eqref{eq:kl-deriv}, we see that in order to know the behavior of the $\KL$ divergence, we need to know how the density of $p_t$ of $\hat{X}_t$ evolves, i.e, we need to know ${\color{BrickRed} \frac{\partial p_t}{\partial t}}(x)$. 
